...
VALIDATE() {
    if [ $2 -ne 0 ]; then
        ...
        echo "$TIMESTAMP [INFO] : Installing the package $1 is success" | tee -a $LOGS_FILE
    fi
}
...
for package in $@
    do
        ...
        if [ $? -eq 0 ]; then
            echo "$TIMESTAMP [INFO]: Required $package is already installed skipping" | tee -a $LOGS_FILE
            #exit 1
        else 
